def fit_linear(filename):
    ...
    y_axis_titles=str_lists.pop()
    x_axis_titles=str_lists.pop()
    str_dict= check_version_transpose_to_dict(str_lists)   #keys ('x','dx','y','dy') values (list of floats)
    ...
    pyplot.errorbar(str_dict['x'],y_dots_to_plot,xerr=str_dict['dx'],yerr=str_dict['dy'],ecolor='blue',fmt='None')
...
def open_read_file (filename):
    ...
    my_file.close()
    return str  # all the data in the file now save as a string variable

def organize_str (str):
    str_lists=[]
    for line in str:
    ...
    return str_lists  # now the data is organized by list of strings lists

def check_version_transpose_to_dict(str_lists):
    ...
    return str_dic # now the data is organized in a dict and the values in each key are floats

def test_dict (str_dic):
    if not len(str_dic['x'])== len(str_dic['y']) == len(str_dic['dx']) == len(str_dic['dy']): #chack if all the keys have the same amount of values
        print('Input file error: Data lists are not the same length.')
        return 1
    for dot in str_dic['dx']: #chack if values in key 'dx' are positive
        if dot<0:
            print('Input file error: Not all uncertainties are positive.')
            return 1
    for dot in str_dic['dy']: #chack if values in key 'dy' are positive
        if dot<0:
            print('Input file error: Not all uncertainties are positive.')
            return 1
    return 0

def calculation_avg (str_dict):
...
def calculation_a_da_b_db(dict_of_avg):
    import math
    dict_of_outputs={}
...
def calculation_chi_square_snd_red(dict_of_outputs,str_dict):
    dict_of_chi_square ={}
    list_new_x=[]
    ...
    return dict_of_chi_square

def calculation_of_the_straight_formula(str_dict,dict_of_outputs):
    y_dots_to_plot=[]
    for every_dot in str_dict['x']:
        newdot = (every_dot *dict_of_outputs['a']) + dict_of_outputs['b']
        y_dots_to_plot.append(newdot)
    return (y_dots_to_plot)  # this lists of dots will be the y dots that will enter to the graph
